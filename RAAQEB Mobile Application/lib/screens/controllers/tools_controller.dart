  ...
  Future<void> fetchRoomData() async {
    ...
    try {
      ...
      userLocation!.forEach((key, value) {
        // Check if the key starts with 'room' and value is a Map
        if (key.startsWith('room') && value is Map) {
          final roomId = key;
          // Try to get the room name, fall back to room ID if name is not available
          final roomName = (value).containsKey('name') 
      ...
      // Debug print to verify rooms are being loaded
      print('Loaded rooms for location $location: ${rooms[location]}');
          ...
          final visibleTools = await Future.wait(
            filteredDocs.map((doc) async {
              if (await shouldShowTool(doc.data())) {
                return Tool.fromMap(doc.data(), id: doc.id); // Include document ID
              }
              return null;
            }),
          );
